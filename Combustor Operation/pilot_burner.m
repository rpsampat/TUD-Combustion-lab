close all
clear all
p_gas = [15;24.3891;46.8891;76.1927;113.172];%gas pressure in mbar from manual
p_air = [15;18.285;39.9087;69.0084;104.06];%air pressure in mbar from manual
P_therm = [0.5,1,1.5,2, 2.5];%thermal power from manual
figure
scatter(p_gas,P_therm)
%% extracting polynomial fit for gas pressure vs thermal power
poly = polyfit(P_therm.',p_gas,2)
pin = polyval(poly,3.1) % gas pressure for a certain thermal power
%% Linear fit to gas pressure and thermal power data from manual
% linear fit seemed to work better than polyfit when tested on pilot burner
% with ignition.
% y-y1=m(x-x1)
y1 = P_therm(1);
...
rho_fuel = 0.78;%kg/m3
AF_stoic = 2*(32+3.76*28)/16;% stoichiometric air fuel ratio for methane-air
figure
for phi = 0.6:0.1:1
