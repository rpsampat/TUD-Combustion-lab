...
class Thermocouple:
    ...
    def convection_thermocouple(self, T_ref,h_fac):
        """
        Convection between thermocouple and external cooling flow modelled as cross flow around cylinder
        :return:
        """
        air = ct.Solution('air.cti')
        air.TP = T_ref, ct.one_atm
        mdot = self.mdot_cool *1.225/60000  #kg/s
        area_conv = self.conv_area_calc_ext()
        vel_tc = h_fac*mdot/(air.density*area_conv)
        Pr = air.viscosity * air.cp/air.thermal_conductivity
        Re_tc = air.density*vel_tc*self.dia_tc/air.viscosity
        Nu_lam = 0.664*(Re_tc**0.5)*(Pr**0.333)
        Nu_turb = 0.037*(Re_tc**0.8)*Pr/(1+2.443*(Re_tc**-0.1)*(Pr**(2.0/3.0)-1))
    ...
    def heat_balance(self, T_wall, area_cond, h_fac):
        """
        A T^4 + B T + C = 0
        :return:
        """
        dx = 0.001
        h = self.convection_thermocouple(T_wall, h_fac)
    ...
    def heat_balance_Twall(self, T_TC, area_cond, h_fac):
        """
        A T^4 + B T + C = 0
        :return:
        """
        dx = 0.0025
        h = self.convection_thermocouple(T_TC, h_fac)
        area_rad_tc = (math.pi / 4.0) * self.dia_tc ** 2.0 + math.pi * self.dia_tc * 0.06
        area_rad_chamber = math.pi * self.dia_chamber * 0.1

        area_conv =  math.pi*self.dia_tc*0.06
        A = self.epsi_wall * self.sigma * area_rad_tc
        B = (self.k_steel / dx) * area_cond
        C = -1.0 * self.epsi_tc * self.sigma * area_rad_tc * (T_TC ** 4.0) +\
            ((-self.k_steel / dx) * area_cond - h * area_conv) * T_TC + \
            h * self.T_air * area_conv
        coeff = [A, 0.0, 0.0, B, C]
        roots = np.roots(coeff)
        root_eff = roots.real[(abs(roots.imag) < 1e-05) & (roots.real > 274)]
        """Q_rad = -1.0 * self.epsi_tc * self.sigma * area_rad_tc * root_eff[0] ** 4.0 \
                + self.epsi_wall * self.sigma * T_wall ** 4.0 * area_rad_tc
        Q_rad_chamber = self.epsi_wall * self.sigma * T_wall ** 4.0 * area_rad_chamber"""
        if len(root_eff) == 0:
    ...
    def heat_balance_fluid(self, T_wall, T_TC, area_cond, h_fac):
        """
        A*Tf + B = 0
        :return:
        """
        ...
        h = self.convection_thermocouple(T_TC, h_fac)
        area_rad_tc = (math.pi / 4.0) * self.dia_tc ** 2.0 + math.pi * self.dia_tc * 0.06
        area_conv = (math.pi / 4.0) * self.dia_tc ** 2.0 + math.pi*self.dia_tc*0.06
        A = h * area_conv
        B =  self.epsi_wall * self.sigma * area_rad_tc * (T_wall ** 4.0) \
            -1.0 * self.epsi_tc * self.sigma * area_rad_tc * (T_TC ** 4.0) \
            - h * area_conv * T_TC
            #+ self.epsi_wall * self.sigma * area_rad_tc * (373.14 ** 4.0) \


        coeff = [A, B]
        #print coeff
        roots = np.roots(coeff)
        root_eff = roots.real[(abs(roots.imag) < 1e-05) & (roots.real > 350)]
        """Q_rad = -1.0 * self.epsi_tc * self.sigma * area_rad_tc * root_eff[0] ** 4.0 \
                + self.epsi_wall * self.sigma * T_wall ** 4.0 * area_rad_tc
        Q_rad_chamber = self.epsi_wall * self.sigma * T_wall ** 4.0 * area_rad_chamber"""
        return root_eff
