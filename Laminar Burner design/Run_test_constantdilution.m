...
power_fuel = 0.1;%kW
% thermal power
P_min = 0.5;%0.1 for laminar
P_max = 2;%0.3 for laminar
p_step = 0.05;%0.05 for laminar
...
vel_orif = zeros(Phi_range,P_Range);
dp_orif = zeros(Phi_range,P_Range);
num_equi = 0;
O2_perc = 20.95;
...
for equivalence_ratio = eq_min:eq_step:eq_max
    ...
    for power_fuel =P_min:p_step:P_max
        ...
        vel_orif(num_equi,num_power) = exhaust.vel_orif;
        dp_orif(num_equi,num_power) = exhaust.dp;
    end
end
...
figure
for i =1:Phi_range
    plot(P_therm(i,:),vel_orif(i,:),'LineWidth',2)
    hold on
end
title(sprintf('Exhaust Orifice Velocity v/s Power'));
xlabel('Power(kW)');
ylabel('Velocity(m/s)');
leg=legend(string(eq_min:eq_step:eq_max), 'Location','northwest');
title(leg,'\phi');
...
figure
for i =1:Phi_range
    plot(P_therm(i,:),dp_orif(i,:),'LineWidth',2)
    hold on
end
title(sprintf('Pressure drop Orifice v/s Power'));
xlabel('Power(kW)');
ylabel('dP (Pa)');
leg=legend(string(eq_min:eq_step:eq_max), 'Location','northwest');
title(leg,'\phi');
